...
import boto3
import logging
from plugins import securitygroup_writes
from plugins import console_logins
from plugins import iam_writes
from plugins import modifyimageattributes
from plugins import cloudtrail
from plugins import route53
from plugins import sqs
...
def lambda_handler(event, context=None):
    ...
    if event.get("action") == "configure":
        logging.warn('Event is a configure event.')
        if context:
    ...
    if 'detail' in event:
        logging.warn('Event is a plugin event.')
...
def configure_events(region):
    ...
    logging.warn('Plugins: {}'.format(PLUGINS))
    for plugin in PLUGINS:
        plugin_instance = plugin['class'](CONFIG)
        logging.warn('Working on {}'.format(plugin_instance.CLOUDWATCH_RULE_NAME))
