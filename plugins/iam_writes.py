    ...
    def match(self, event):
        if event.get("source") == "aws.iam":
            if not startswithany(
                    ...
                    ["List", "Get", "Delete", "Detach", "Generate", "Remove"]) \
                    and "jenkins" not in event['detail']['userIdentity']['arn'] \
                    and "gitlamb" not in event['detail']['userIdentity']['arn'] \
                    and ".com" not in event['detail']['sourceIPAddress']:
                return True
        return False
            ...
            "attachments": [
                {
                    "fallback": text,
                    "color": "#36a64f",
                    "title": "AWS IAM Modification - {} {} ({})".format(event['detail']['eventName'],
                                                                        self.config['ACCOUNTS'][event['account']][
                                                                            'name'], event['account']),
                    "text": text,
